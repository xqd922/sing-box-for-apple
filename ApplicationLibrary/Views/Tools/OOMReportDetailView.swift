...
public struct OOMReportDetailView: View {
    ...
    private var manager: OOMReportManager {
    ...
    #if !os(tvOS)
        private func saveReport(includeConfig: Bool, includeLog: Bool) async {
            do {
        ...
        private func shareReport(includeConfig: Bool, includeLog: Bool) async {
            do {
                let zipURL = try await createReportZip(
                    reportID: report.id, fileURL: report.fileURL,
                    cacheSubdirectory: ReportType.oom.directoryName, includeConfig: includeConfig, includeLog: includeLog
    ...
    public var body: some View {
        ...
        .alert($alert)
            ...
            .sheet(isPresented: $sharePopupPresented, onDismiss: {
                ...
                switch action {
                case .save:
                    Task { await saveReport(includeConfig: includeConfig, includeLog: includeLog) }
                case .share:
                    Task { await shareReport(includeConfig: includeConfig, includeLog: includeLog) }
                case .none:
                    break
                }
            }) {
